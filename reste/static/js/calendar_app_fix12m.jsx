// build: fix12m
console.log('calendar_app_fix12m.jsx loaded');
...
function getAllById(id){
  try{
    const a = JSON.parse(localStorage.getItem('busops:orders_v1')||'[]');
    return (Array.isArray(a) ? a : []).filter(x=> String(x.id)===String(id));
  }catch{ return []; }
}
function replaceAllById(id, newItems){
  try{
    const a = JSON.parse(localStorage.getItem('busops:orders_v1')||'[]');
    const rest = (Array.isArray(a) ? a : []).filter(x=> String(x.id)!==String(id));
    const merged = rest.concat(newItems);
    localStorage.setItem('busops:orders_v1', JSON.stringify(merged));
    localStorage.setItem('orders_fallback_store_v1', JSON.stringify(merged));
    window.dispatchEvent(new CustomEvent('orders:changed'));
  }catch{}
}
async function persistRangeOnServer(id, start_date, end_date, vehicle_plate){
  try{
    await fetch('/orders/api/update_range', {
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ id, start_date, end_date, vehicle_plate })
    });
  }catch{}
}
...
// Helpers за визуален span (свързани блокове)
function sameDay(a,b){ return a.getFullYear()===b.getFullYear() && a.getMonth()===b.getMonth() && a.getDate()===b.getDate(); }
function isPrevDay(d1, d2){ // is d1 the day before d2?
  const n1 = new Date(d1.getFullYear(), d1.getMonth(), d1.getDate());
  const n2 = new Date(d2.getFullYear(), d2.getMonth(), d2.getDate());
  return (n1.getTime() + 24*3600*1000) === n2.getTime();
}
...
function MonthView({anchor, assignments, dnd, ordersById, lanes=[], busFilter='Всички', mode='equal', onSelect, selectedId}){
  ...
  // DnD drop handler: shift whole span to new date & assign lane
  function dropOnCell(date, lane){
    return (e)=>{
      e.preventDefault();
      const id = e.dataTransfer.getData("text/plain");
      if(!id) return;
      // determine span for this id
      const all = getAllById(id);
      if(!all.length) return;
      const dates = all.map(x=> parseISO(x.date)).filter(Boolean).sort((a,b)=>a-b);
      const start = dates[0], end = dates[dates.length-1];
      const duration = Math.round((end - start) / (24*3600*1000)) + 1;
      const newStart = new Date(date.getFullYear(), date.getMonth(), date.getDate());
      const newEnd = addDays(newStart, duration-1);

      // build new flat entries for this id
      const [sh, sm] = ((all[0].startTime||'08:00').split(':').map(n=>parseInt(n||0,10)));
      const [eh, em] = ((all[0].endTime||'10:00').split(':').map(n=>parseInt(n||0,10)));
      const title = all[0].title || ('Поръчка #'+id);
      ...
      for(let i=0;i<duration;i++){
        ...
        const ymd = isoDate(d);
        fresh.push({ id:String(id), date: ymd, startTime: hh(sh)+':'+hh(sm), endTime: hh(eh)+':'+hh(em), title, vehicle_plate: plate });
      }

      replaceAllById(id, fresh);
      persistRangeOnServer(String(id), isoDate(newStart), isoDate(newEnd), plate);
      window.dispatchEvent(new CustomEvent("orders:changed"));
  ...
  function renderItem(a, listForPrevDaySameLane, listForNextDaySameLane){
    const isJoinedLeft  = listForPrevDaySameLane.some(x=> String(x.id)===String(a.id));
    const isJoinedRight = listForNextDaySameLane.some(x=> String(x.id)===String(a.id));
    const cls = [
      "text-[10px] px-1.5 py-0.5 border cursor-move truncate bg-amber-50",
      isJoinedLeft  ? "rounded-l-none" : "rounded-l",
      isJoinedRight ? "rounded-r-none" : "rounded-r",
      "border-amber-300"
    ].join(" ");
    return (
      <div key={a.id}
          ...
          {lanesToShow.map((lane,ri) => (
            <React.Fragment key={lane}>
              ...
              {days.map((d,i)=>{
                ...
                const prevList = (byDayLane.get(isoDate(addDays(d,-1)))?.get(lane)) || [];
                const nextList = (byDayLane.get(isoDate(addDays(d, 1)))?.get(lane)) || [];
                ...
                return (
                  <div key={i} className={cellBase + " " + zebra} onDragOver={lane==='НЕАКТИВНИ:' ? undefined : dnd.allowDrop} onDrop={dropOnCell(d, lane)}>
                    <div className="space-y-1">
                      {list.map(a=> renderItem(a, prevList, nextList))}
                    </div>
                  </div>
                );
              })}
...
// ---- WeekView (подобно поведение за span) ----
...
// ---- DayView (еднодневно поведение остава) ----
function DayView({anchor, assignments, dnd, ordersById, lanes=[], mode='equal', onSelect, selectedId}){
  ...
  function dropOnCell(hour, lane){
    return (e)=>{
      ...
      const id = e.dataTransfer.getData("text/plain");
      if(!id) return;
      // изместване към нов ден, запазвайки продължителност
      const all = getAllById(id);
      if(!all.length) return;
      const dates = all.map(x=> parseISO(x.date)).filter(Boolean).sort((a,b)=>a-b);
      const start = dates[0], end = dates[dates.length-1];
      const duration = Math.round((end - start) / (24*3600*1000)) + 1;
      const newStart = new Date(day.getFullYear(), day.getMonth(), day.getDate());
      const newEnd = addDays(newStart, duration-1);
                    ...
                    <div className="space-y-1">
                      {list.map(a=>(
                        <div key={a.id} draggable onDragStart={(e)=>dnd.onDragStart(e, a.id)} onDragEnd={dnd.onDragEnd}
                             onClick={()=>onSelect && onSelect(a.id)}
                             className={"text-[10px] px-1.5 py-0.5 rounded bg-amber-50 border border-amber-300 cursor-move truncate " + (selectedId===a.id?"outline outline-2 outline-amber-400":"")}
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="bg-slate-50 rounded-lg p-2">
                ...
                <div>Марж: {(((selectedOrder.price||0)-Object.values((selectedOrder.planned && selectedOrder.planned.costs) || {}).reduce((s,v)=>s+Number(v||0),0))).toFixed(2)} €</div>
              </div>
              <div className="bg-slate-50 rounded-lg p-2">
                ...
                <div>Разходи: {Object.values((selectedOrder.actual and selectedOrder.actual.costs) || {}).reduce((s,v)=>s+Number(v||0),0).toFixed(2)} €</div>
                <div>Марж: {(((selectedOrder.actual and selectedOrder.actual.revenue != null ? selectedOrder.actual.revenue : (selectedOrder.price||0)) - Object.values((selectedOrder.actual and selectedOrder.actual.costs) || {}).reduce((s,v)=>s+Number(v||0),0))).toFixed(2)} €</div>
              </div>
            </div>
