...
def _portal_ticket_qr_svg_response(
    ...
    """
    Генерира QR код за конкретен билет.

    Backward-compatible:
    - ако е подадено seat_no, QR кодът е за конкретното място;
    - ако seat_no липсва, използва първото FINAL място.

    Така старите линкове към /portal/ticket/qr продължават да работят,
    а новите билети могат да имат отделен QR код за всяко място.
    """
    booking, redirect_resp = _portal_booking_or_redirect(
        request,
        db,
    )
    ...
    selected_seat = _resolve_requested_ticket_seat(
        booking,
        seat_no,
    )
    ...
    if (
        not selected_seat
        or not _ticket_qr_available(booking)
    ):
        return Response(status_code=404)
    ...
    if getattr(booking, "trip_id", None):
        trip = (
            db.query(Trip)
            .filter(Trip.id == booking.trip_id)
            .first()
        )

    passenger = _find_ticket_passenger_for_seat(
        db,
        booking,
        selected_seat,
    )
    ...
    if as_attachment:
        headers["Content-Disposition"] = (
            f'attachment; '
            f'filename="ticket-qr-{booking.external_id}-seat-{selected_seat}.svg"'
        )
...
def _ticket_seat_sort_key(value: str):
    """
    Сортира числовите места правилно:
      2, 3, 10, 11
    вместо:
      10, 11, 2, 3
    """
    text = _normalize_ticket_seat_no(value)
...
def _booking_selected_seats(
    ...
    Поддържа и двата начина на извикване:
      _booking_selected_seats(booking)
      _booking_selected_seats(db, booking)

    Когато има db session, чете директно от BookingSeat.
    Иначе използва relationship-а booking.seats.
    """
    if booking is None:
        db = None
    ...
    if db is not None and getattr(booking, "id", None):
        try:
            rows = (
                db.query(BookingSeat)
                .filter(
                    BookingSeat.booking_id == int(booking.id),
                    BookingSeat.is_final.is_(True),
                    BookingSeat.seat_no.isnot(None),
                )
                .order_by(BookingSeat.id.asc())
                .all()
    ...
    if not rows:
        rows = list(getattr(booking, "seats", []) or [])

    out: list[str] = []
    seen: set[str] = set()

    for row in rows:
        seat_no = _normalize_ticket_seat_no(
            getattr(row, "seat_no", None)
        )
...
def _portal_ticket_payload(
    ...
    """
    Генерира payload за QR кода на конкретен билет.

    Поддържа и двата начина на извикване:
      _portal_ticket_payload(booking, "50")
      _portal_ticket_payload(booking=booking, selected_seat="50")
      _portal_ticket_payload(booking=booking, seat_no="50")

    Това е важно, защото различни части от приложението използват
    selected_seat или seat_no. И двата варианта вече работят.
    """
    ...
    passenger_name = (
        f"{getattr(booking, 'first_name', None) or ''} "
        f"{getattr(booking, 'last_name', None) or ''}"
    ).strip()
    ...
    payload["sig"] = _sign_ticket_payload(
        payload
    )
...
def _portal_ticket_payload_text(
    booking: Booking,
    selected_seat: str | None = None,
    passenger: TripPassenger | None = None,
    trip: Trip | None = None,
    ...
    """
    Връща QR payload като JSON текст.

    Използва се от PDF билета и може да се използва и от SVG route-а.
    Поддържа selected_seat и seat_no за backward compatibility.
    """
...
def _portal_qr_drawing(
    payload_text: str,
    size: float = 42 * mm,
) -> Drawing:
    """
    Създава QR drawing, който може да се използва:
      - като SVG за HTML страницата;
      - в PDF билета.
    """
...
def _portal_qr_svg_bytes(
    payload_text: str,
) -> bytes:
    drawing = _portal_qr_drawing(
        payload_text,
        size=180,
    )

    svg_data = renderSVG.drawToString(
        drawing
    )
...
def _ticket_decimal(value) -> Decimal | None:
    """
    Безопасно преобразува стойност към Decimal за визуализация на цени.
    """
    if value in (None, ""):
        return None
...
def _ticket_positive_qty(value) -> int:
    try:
        qty = int(value or 0)
    except Exception:
        qty = 0
...
def _ticket_money_text(value, currency: str | None) -> str:
    amount = _ticket_decimal(value)
    if amount is None:
        return ""

    currency_text = str(currency or "").strip().upper()
    return f"{amount:.2f} {currency_text}".strip()


def _ticket_booking_total_text(booking: Booking) -> str:
    return _ticket_money_text(
        getattr(booking, "total", None),
        getattr(booking, "currency", None),
    )


def _ticket_line_unit_text(line: BookingTicketLine, qty: int) -> str:
    """
    Връща цената за ЕДИН билет от конкретен BookingTicketLine.

    Ако редът съдържа обща стойност за qty > 1, тя се разделя
    на броя билети. Така всеки отделен билет получава собствена цена.
    """
    qty_decimal = Decimal(str(max(int(qty or 1), 1)))

    line_total_uah = _ticket_decimal(
        getattr(line, "line_total_uah", None)
    )
    line_total_eur = _ticket_decimal(
        getattr(line, "line_total_eur", None)
    )

    dual_parts: list[str] = []

    if line_total_uah is not None:
        dual_parts.append(
            _ticket_money_text(
                line_total_uah / qty_decimal,
                "UAH",
            )
        )

    if line_total_eur is not None:
        dual_parts.append(
            _ticket_money_text(
                line_total_eur / qty_decimal,
                "EUR",
            )
        )

    if dual_parts:
        return " / ".join(part for part in dual_parts if part)

    line_total = _ticket_decimal(
        getattr(line, "line_total", None)
    )

    if line_total is not None:
        return _ticket_money_text(
            line_total / qty_decimal,
            getattr(line, "currency", None),
        )

    # Backward-compatible fallback, ако importer-ът пази unit price отделно.
    for attr_name in (
        "unit_price",
        "price",
        "amount",
    ):
        unit_value = _ticket_decimal(
            getattr(line, attr_name, None)
        )
...
def _portal_ticket_price_units(
    ticket_lines: list[BookingTicketLine] | None,
) -> list[dict]:
    """
    Разгръща ценовите редове до отделни единични билети.

    Пример:
      child, qty=1, line_total=70 EUR
      adult, qty=1, line_total=100 EUR

    става:
      [
        {ticket_type: child, ticket_price_text: 70.00 EUR},
        {ticket_type: adult, ticket_price_text: 100.00 EUR},
      ]

    Няма директна връзка seat -> ticket line в базата. Затова единичните
    цени се свързват последователно със сортираните места. Порядъкът не
    променя общата стойност и не участва в QR payload-а.
    """
    units: list[dict] = []

    for line in list(ticket_lines or []):
        qty = _ticket_positive_qty(
            getattr(line, "qty", None)
        )

        ticket_type = str(
            getattr(line, "ticket_type_raw", None)
            or getattr(line, "ticket_type", None)
            or ""
        ).strip()

        unit_price_text = _ticket_line_unit_text(
            line,
            qty,
        )
        ...
        for _ in range(qty):
            units.append({
                ...
                "ticket_price_text": unit_price_text,
            })

    return units


def _ticket_decimal_or_none(value) -> Decimal | None:
    if value in (None, ""):
        return None
...
def _ticket_money_text(
    amount,
    currency: str | None,
) -> str | None:
    parsed_amount = _ticket_decimal_or_none(amount)
    ...
    return f"{parsed_amount:.2f} {currency_text}"


def _ticket_line_unit_price_text(
    line: BookingTicketLine,
) -> str | None:
    """
    Връща цената за ЕДИН билет от даден BookingTicketLine.
    ...
    Резултат:
      70.00 EUR за всеки от двата билета.
    """
    qty = _ticket_qty_int(
        getattr(
            line,
    ...
    # Стандартен вариант: една валута.
    line_total = _ticket_decimal_or_none(
        getattr(
            line,
            "line_total",
            None,
    ...
    # Fallback: EUR стойност при dual-currency ред.
    line_total_eur = _ticket_decimal_or_none(
        getattr(
            line,
            "line_total_eur",
            None,
    ...
    # Fallback: UAH стойност, когато няма EUR.
    line_total_uah = _ticket_decimal_or_none(
        getattr(
            line,
            "line_total_uah",
            None,
    ...
    # Допълнителни fallback полета, ако моделът ги има.
    for attr_name in (
        "unit_price",
        "price",
        "amount",
...
def _expanded_ticket_line_prices(
    ticket_lines: list[BookingTicketLine],
) -> list[str | None]:
    """
    Разгъва ticket lines до цена за всяко отделно място.

    Пример:
      дете qty=1, total=70 EUR
      възрастен qty=1, total=100 EUR

    Резултат:
      ["70.00 EUR", "100.00 EUR"]
    """
    expanded_prices: list[str | None] = []

    for line in ticket_lines or []:
        qty = _ticket_qty_int(
            getattr(
                line,
                "qty",
        ...
        unit_price_text = _ticket_line_unit_price_text(
            line
        )
...
def _normalize_ticket_seat_no(value) -> str | None:
    """
    Нормализира номер на място.
    ...
    seat_no = str(value).strip()
    if not seat_no:
        return None

    if re.fullmatch(r"\d+\.0+", seat_no):
        seat_no = seat_no.split(".", 1)[0]

    return seat_no or None
...
def _resolve_requested_ticket_seat(
    ...
    """
    Определя за кое място трябва да бъде показан билетът.

    - Ако URL съдържа ?seat_no=50, връща 50 само ако мястото
      действително принадлежи на booking-а.
    - Ако параметър липсва, връща първото място за backward compatibility.
    """
    selected_seats = [
        normalized
        for normalized in (
            _normalize_ticket_seat_no(item)
            for item in _booking_selected_seats(booking)
        )
        if normalized
    ]

    requested_seat = _normalize_ticket_seat_no(
        seat_no
    )

    if requested_seat:
        if requested_seat in selected_seats:
...
def _ticket_passenger_effective_seat(
    passenger: TripPassenger,
) -> str | None:
    """
    Връща актуалното място на TripPassenger.

    manual_seat_no е с приоритет, защото съдържа промени,
    направени от администратора.
    """
    return _normalize_ticket_seat_no(
        getattr(
            passenger,
            "manual_seat_no",
            None,
...
def _find_ticket_passenger_for_seat(
    ...
    """
    Намира правилния TripPassenger ред за конкретното място.

    Това е важно при booking с повече от един билет:
      Място 50 -> отделен билет и QR
      Място 51 -> отделен билет и QR

    Приоритет:
      1) linked TripPassenger по booking_id + точно съвпадение на място
      2) linked TripPassenger по стабилен индекс
      3) fallback по trip_id + external_id + точно място
      4) fallback по стабилен индекс
      5) единствен passenger row, ако няма двусмислие
    """
    normalized_seat = _normalize_ticket_seat_no(
        seat_no
    )
    ...
    try:
        seat_index = selected_seats.index(
            normalized_seat
        )
    except ValueError:
        seat_index = None
    ...
    # -------------------------------------------------------
    # 1) Основен вариант: TripPassenger вече е linked към booking_id
    # -------------------------------------------------------
    linked_passengers = (
        db.query(TripPassenger)
        .filter(
        ...
        .order_by(
            TripPassenger.id.asc()
        )
        .all()
    )

    try:
        linked_passengers = sorted(
            linked_passengers,
    ...
    for passenger in linked_passengers:
        if (
            _ticket_passenger_effective_seat(
                passenger
            )
            == normalized_seat
        ):
    ...
    # Ако seat sync още не е нанесен върху TripPassenger,
    # използваме стабилното съответствие по индекс.
    if (
        seat_index is not None
        and seat_index < len(linked_passengers)
    ):
        return linked_passengers[
            seat_index
        ]

    # -------------------------------------------------------
    # 2) Fallback: booking_id още липсва, но TripPassenger
    #    може да бъде открит по trip_id + external_id
    # -------------------------------------------------------
    resolved_trip_id = getattr(
        booking,
        "trip_id",
        None,
    )
    ...
    if not resolved_trip_id:
        try:
            resolved_trip_id = _resolve_booking_trip_id(
                db,
                booking,
            )
        except Exception:
    ...
    external_id_key = _booking_external_id_key(
        booking
    )

    fallback_passengers: list[TripPassenger] = []
        ...
        for passenger in trip_passengers:
            try:
                passenger_external_id = (
                    _trip_passenger_external_id_key(
                        passenger,
            ...
            if (
                ...
                fallback_passengers.append(
                    passenger
                )

    try:
        fallback_passengers = sorted(
            fallback_passengers,
            key=_trip_passenger_sort_key,
        )
    except Exception:
    ...
    for passenger in fallback_passengers:
        if (
            _ticket_passenger_effective_seat(
                passenger
            )
    ...
    if (
        seat_index is not None
        and seat_index < len(fallback_passengers)
    ):
    ...
    # -------------------------------------------------------
    # 3) Безопасен последен fallback само при един ред
    # -------------------------------------------------------
    if len(linked_passengers) == 1:
        return linked_passengers[0]

    if len(fallback_passengers) == 1:
        return fallback_passengers[0]
...
def _portal_ticket_payload_text(
    ...
    """
    Генерира JSON текста за QR кода.

    Цената на индивидуалния билет умишлено НЕ се добавя към QR payload-а.
    Тя остава само информативна част от HTML/PDF билета.
    """
    payload = _portal_ticket_payload(
        booking=booking,
        selected_seat=selected_seat,
...
def _portal_ticket_items(
    db: Session,
    booking: Booking,
    ticket_lines: list[BookingTicketLine] | None = None,
) -> list[dict]:
    """
    Подготвя отделен билет за всяко FINAL място.

    QR payload-ът не се усложнява:
    - всеки билет продължава да има собствен seat_no;
    - цената е само информативна част от HTML/PDF билета;
    - цената не се добавя към QR payload-а.
    """
    if ticket_lines is None:
        ticket_lines = (
            db.query(BookingTicketLine)
            .filter(
                BookingTicketLine.booking_id == booking.id
            )
            .order_by(
                BookingTicketLine.id.asc()
            )
            .all()
        )

    selected_seats = _booking_selected_seats(
        booking
    )

    expanded_prices = _expanded_ticket_line_prices(
        ticket_lines
    )

    booking_total_text = _ticket_money_text(
        getattr(
            booking,
            "total",
            None,
    ...
    for index, seat_no in enumerate(
        selected_seats
    ):
        normalized_seat = _normalize_ticket_seat_no(
            seat_no
        ...
        ticket_price_text = (
            expanded_prices[index]
            if index < len(expanded_prices)
            else None
        )

        # При booking само с едно място може безопасно да се използва total.
        if (
            not ticket_price_text
            and len(selected_seats) == 1
        ):
            ticket_price_text = booking_total_text

        items.append(
            {
                "seat_no": normalized_seat,
                "ticket_ref": payload.get(
                    "ticket_ref"
                ),
                "ticket_price_text": (
                    ticket_price_text
                ...
                "qr_url": (
                    "/portal/ticket/qr"
                    f"?seat_no={encoded_seat}"
                ),
                "qr_download_url": (
                    "/portal/ticket/qr/download"
                    f"?seat_no={encoded_seat}"
                ),
                "pdf_url": (
                    "/portal/ticket/pdf"
...
def portal_ticket_page(
    ...
    if not _can_portal_view_ticket(booking):
        return RedirectResponse(
            url="/portal?cash_err=ticket",
            status_code=303,
        )

    service_label = _booking_service_label(
        booking,
    )
    ...
    tickets = _portal_ticket_items(
        db=db,
        booking=booking,
        ticket_lines=ticket_lines,
    )

    stop_points = _booking_stop_points(
        booking
    )


    qr_available = _ticket_qr_available(
        booking,
    )

    tickets = (
        _portal_ticket_items(
            db,
            booking,
        )
        if selected_seats
    ...
    first_departure_time = _extract_first_departure_time(
        getattr(
            booking,
            "time_range_raw",
            None,
    ...
    departure_time_label = (
        first_departure_time.strftime("%H:%M")
        if first_departure_time
        else None
    )
...
def portal_ticket_pdf(
    ...
    if not _can_portal_view_ticket(
        booking
    ):
        return RedirectResponse(
            url="/portal?cash_err=ticket",
    ...
    ticket_items = _portal_ticket_items(
        db=db,
        booking=booking,
        ticket_lines=ticket_lines,
    )
    ...
    current_ticket = next(
        (
            ...
            if item.get("seat_no") == selected_seat
        ),
        {},
    )
    ...
    pdf.drawString(
        card_x + 12 * mm,
        top - 7 * mm,
        f"Unique ID: {booking.external_id}",
    )
    ...
    passenger_name = (
        f"{booking.first_name or ''} "
        f"{booking.last_name or ''}"
    ).strip() or "-"

    passenger_route = (
        f"{booking.route_from or '-'} "
        f"-> "
        f"{booking.route_to or '-'}"
    )

    seat_label = (
        selected_seat
        or "Assigned later by dispatch"
    )
    ...
    if (
        ...
        qr_data = _portal_ticket_payload_text(
            booking,
            selected_seat,
        )
        ...
        pdf.drawString(
            qr_box_x,
            qr_box_y + 22 * mm,
            "QR will become available",
        )

        pdf.drawString(
            qr_box_x,
            qr_box_y + 16 * mm,
            "after a final seat is assigned.",
        )
    ...
    pdf.drawString(
        card_x + 12 * mm,
        card_y + 8 * mm,
        "Generated from passenger portal",
    )
    ...
    filename = (
        f"ticket-{booking.external_id}"
        f"-seat-{selected_seat or 'unknown'}.pdf"
    )
...
def _driver_scan_effective_seat(
    ...
    Приоритет:
      ...
      3) първото FINAL BookingSeat място за backward compatibility
    """
    payload = payload or {}
    ...
    values = [
        ...
        payload.get("seat_no"),
        _booking_selected_seat(booking) if booking else None,
    ]
