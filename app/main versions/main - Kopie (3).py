...
def _booking_is_cancelled(booking: Booking) -> bool:
    return (getattr(booking, "booking_status", None) or "").strip().lower() == "cancelled"
...
def _build_dashboard_direction_payload(
    ...
    for p in trip_passengers:
        ...
        if seat_no:
            taken_seats.append(seat_no)

        passengers_payload.append({
            "id": p.id,
            "first_name": getattr(p, "first_name", None),
            "last_name": getattr(p, "last_name", None),
            "passenger_name": f"{getattr(p, 'first_name', '') or ''} {getattr(p, 'last_name', '') or ''}".strip(),
    ...
    return {
        "present": True,
        "label": label,
        "issue_count": len(bank_paypal_missing_proof_le72) + len(cash_not_confirmed_le72),
        ...
        "taken_seats": sorted(set(taken_seats), key=lambda x: int(x) if str(x).isdigit() else 9999),
...
def portal_cancellation_page(request: Request, db: Session = Depends(get_db)):
    ...
    is_cancelled_booking = _booking_is_cancelled(booking)
    has_active_cancellation_request = bool(
        existing_cancellation
        and (existing_cancellation.admin_status or "").strip().lower() in {"pending", "approved", "processed"}
    )

    cancel_button_disabled = bool(
        is_cancelled_booking
        or has_active_cancellation_request
        or not bool(cancel_policy.get("allowed"))
    )
    ...
    return templates.TemplateResponse(
        ...
        "portal/cancellation.html",
        {
            ...
            "is_cancelled_booking": is_cancelled_booking,
            "has_active_cancellation_request": has_active_cancellation_request,
            "cancel_button_disabled": cancel_button_disabled,
        },
    )
...
@app.post("/admin/bookings/cancellations/{cancellation_id}/approve")
def admin_booking_cancellation_approve(
    cancellation_id: int,
    request: Request,
    db: Session = Depends(get_db),
):
    ...
    cancellation = (
        db.query(BookingCancellation)
        .filter(BookingCancellation.id == cancellation_id)
        .first()
    )
    if not cancellation:
    ...
    booking = (
        db.query(Booking)
        .filter(Booking.id == cancellation.booking_id)
        .first()
    )
    if not booking:
        raise HTTPException(404, "Booking not found")

    cancellation.admin_status = "approved"
    ...
    try:
        _sync_booking_seats_to_trip_passengers(db, booking.id, [])
    except Exception:
        pass

    try:
        _set_booking_trip_passengers_admin_seat_lock(db, booking.id, False)
    except Exception:
        pass

    note_line = f"[ADMIN] Cancellation approved at {datetime.utcnow().strftime('%d.%m.%Y %H:%M UTC')}"
    ...
    return RedirectResponse(
        url="/admin/bookings/cancellations?ok=approved",
        status_code=303,
    )
...
def admin_bookings_dashboard_page(request: Request, db: Session = Depends(get_db)):
    ...
    for date_key, dirs in day_direction_map.items():
        ...
        for dir_key, bucket in dirs.items():
            issue_count = (
                len(bucket["bank_paypal_missing_proof_le72"])
                + len(bucket["bank_paypal_missing_proof_gt72"])
                + len(bucket["cash_not_confirmed_le72"])
                + len(bucket["cash_not_confirmed_gt72"])
                + len(bucket["dispatcher_seat_assignment_needed"])
            )

            dashboard_calendar_data[date_key][dir_key] = {
                "label": bucket["label"],
                ...
                "taken_seats": sorted(bucket["taken_seats"], key=lambda s: int(str(s))),
            }
...
def portal_seats_page(request: Request, db: Session = Depends(get_db)):
    ...
    taken_seats = sorted(_service_taken_seats(db, booking, exclude_booking_id=booking.id))
...
def portal_seats_state(request: Request, db: Session = Depends(get_db)):
    ...
    if not _ensure_booking_has_service(booking):
        raise HTTPException(status_code=400, detail="No seat service")

    resolved_trip_id = getattr(booking, "trip_id", None) or _resolve_booking_trip_id(db, booking)
    if not resolved_trip_id:
        raise HTTPException(status_code=400, detail="No trip")
    booking.trip_id = resolved_trip_id

    if _booking_has_admin_locked_seat(db, booking):
        raise HTTPException(status_code=409, detail="Seat locked by admin")
...
def portal_assign_seats(
    request: Request,
    seat_nos: list[str] = Form([]),
    db: Session = Depends(get_db),
):
    booking, redirect_resp = _portal_booking_or_redirect(request, db)
    if redirect_resp:
    ...
    if _booking_selected_seats(booking) and not _can_portal_change_seat(booking):
        return RedirectResponse(url="/portal?seat_err=change_locked", status_code=303)
    ...
    for seat in seat_nos or []:
        ...
        if seat_no not in allowed:
            return RedirectResponse(url="/portal/seats?seat_err=invalid", status_code=303)
        seen.add(seat_no)
        normalized.append(seat_no)

    if passenger_count <= 0:
        return RedirectResponse(url="/portal/seats?seat_err=service", status_code=303)

    if len(normalized) != passenger_count:
        return RedirectResponse(url="/portal/seats?seat_err=missing", status_code=303)

    taken_seats = _service_taken_seats(db, booking, exclude_booking_id=booking.id)
    for seat_no in normalized:
        if seat_no in taken_seats:
...
def portal_select_seat(
    ...
    draft = list(selected_seats)
    if seat_no not in draft:
        if len(draft) < passenger_count:
            draft.append(seat_no)
        else:
            draft[-1] = seat_no
    ...
    return RedirectResponse(url="/portal/seats?seat_ok=selected", status_code=303)
...
def portal_auto_assign_seat(
    ...
    if need_count <= 0:
        ...
        db.commit()
        return RedirectResponse(url="/portal/seats?seat_ok=auto", status_code=303)
    ...
    final_selection = list(selected_seats)
    for seat_no in free_seats:
        if seat_no not in final_selection:
            final_selection.append(seat_no)

    _apply_booking_seat_assignment(
        db=db,
        booking=booking,
        seat_nos=final_selection,
        selection_mode="auto",
    )
    db.commit()
