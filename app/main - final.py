...
def _dashboard_booking_base_date(
    booking: Booking,
    trip_by_id: dict[int, Trip] | None = None,
) -> date | None:
...
def _dashboard_stop_country_group(value: str | None) -> str | None:
    ...
    at_keys = {"wien", "stpolten", "linz", "wels", "salzburg", "innsbruck", "bruck", "graz", "klagenfurt", "villach"}
...
def _dashboard_direction_meta_for_booking(
    booking: Booking,
    trip_by_id: dict[int, Trip] | None = None,
) -> dict:
    trip_id = getattr(booking, "trip_id", None)
...
def _booking_has_admin_locked_seat(db: Session, booking: Booking) -> bool:
    ...
    return any(bool(getattr(p, "seat_locked_by_admin", False)) for p in rows)
...
def _build_dashboard_direction_payload(
    ...
    trip_passenger_count = len(passengers_payload)
    fallback_confirmed_count = 0

    for booking in list(items or []):
        booking_id = int(getattr(booking, "id", 0) or 0)
        booking_status_norm = str(getattr(booking, "booking_status", "") or "").strip().lower()
        payment_status_norm = str(getattr(booking, "payment_status", "") or "").strip().lower()
        final_seat = str(getattr(booking, "_dashboard_final_seat", "") or "").strip()
        passenger_count = int(getattr(booking, "_dashboard_passenger_count", 1) or 1)

        if final_seat:
            taken_seats.add(final_seat)

        is_confirmed_like = (
            booking_status_norm in {"confirmed", "paid"}
            or payment_status_norm == "paid"
            or bool(final_seat)
        )
        if not is_confirmed_like:
            continue

        if booking_id not in represented_booking_ids:
            first_name = (getattr(booking, "first_name", None) or "").strip() or None
            last_name = (getattr(booking, "last_name", None) or "").strip() or None
            passenger_name = f"{first_name or ''} {last_name or ''}".strip() or "—"
            phone = (getattr(booking, "phone", None) or "").strip() or None
            ...
            passengers_payload.append({
                ...
                "phone": phone,
                "seat_no": final_seat or None,
                "seat_locked_by_admin": False,
            })
            fallback_confirmed_count += max(1, passenger_count)

    if total_confirmed is None:
        if trip_passenger_count or fallback_confirmed_count:
            total_confirmed = trip_passenger_count + fallback_confirmed_count
        else:
            total_confirmed = len(items or [])
    ...
    return {
        ...
        "issue_count": (
            ...
            + len(bank_paypal_missing_proof_gt72)
            + len(cash_not_confirmed_le72)
            + len(cash_not_confirmed_gt72)
...
def _booking_is_cancelled_or_pending_cancellation(booking: Booking) -> bool:
    if not booking:
         return False
 
    status = str(getattr(booking, "booking_status", "") or "").strip().lower()
...
def _service_taken_seats(
    ...
    """
    Взима всички FINAL заети места за същата дата + bus_from + bus_to.
    Гледа BookingSeat, не TripPassenger.
    """
    service_key = _booking_service_key(booking)
    if not service_key:
        return set()
    ...
    rows = (
        ...
        .filter(BookingSeat.is_final == True)
        .all()
    )

    taken: set[str] = set()

    for seat_row, b in rows:
        if exclude_booking_id is not None and b.id == exclude_booking_id:
            continue

        other_key = _booking_service_key(b)
        if not other_key:
            continue
        ...
        seat_no = (seat_row.seat_no or "").strip()
        if seat_no:
            taken.add(seat_no)
...
def admin_booking_assign_seat(
    ...
    if not resolved_trip_id:
        return RedirectResponse(
            url=f"/admin/bookings/{booking_id}?seat_admin_err=service",
            status_code=303,
        )
    booking.trip_id = resolved_trip_id
...
def admin_bookings_dashboard_page(request: Request, db: Session = Depends(get_db)):
    ...
    pending_cancellation_booking_ids = sorted({
        int(x.booking_id)
        for x in pending_cancellation_rows
        if getattr(x, "booking_id", None) is not None
    })

    pending_cancellation_booking_map: dict[int, Booking] = {}
    if pending_cancellation_booking_ids:
        pending_cancellation_booking_map = {
            int(b.id): b
            for b in db.query(Booking).filter(Booking.id.in_(pending_cancellation_booking_ids)).all()
        }
    ...
    for x in pending_cancellation_rows:
        booking = None
        if getattr(x, "booking_id", None) is not None:
            booking = pending_cancellation_booking_map.get(int(x.booking_id))
        ...
        requested_str = (
            x.requested_at.strftime("%d.%m.%Y %H:%M")
            if getattr(x, "requested_at", None)
            else "—"
        )
        travel_str = (
            x.travel_at.strftime("%d.%m.%Y %H:%M")
            if getattr(x, "travel_at", None)
            else "—"
        )

        pending_cancellations.append({
            "id": x.id,
            "booking_id": getattr(x, "booking_id", None),
            "external_id": getattr(x, "external_id", None) or (getattr(booking, "external_id", None) if booking else None),
            "passenger_name": passenger_name,
            "route": route,
            "requested_str": requested_str,
            "travel_str": travel_str,
            "refund_percent": int(getattr(x, "refund_percent", 0) or 0),
            "url": "/admin/bookings/cancellations",
        })
    ...
    for booking in all_bookings:
        ...
        if resolved_trip_id:
            booking.trip_id = resolved_trip_id
            if int(resolved_trip_id) not in trip_by_id:
                trip = db.query(Trip).filter(Trip.id == resolved_trip_id).first()
                if trip:
                    trip_by_id[int(resolved_trip_id)] = trip
        ...
        if resolved_trip_id:
            bucket["trip_ids"].add(int(resolved_trip_id))
        ...
        setattr(
            booking,
            "_dashboard_passenger_count",
            max(1, int(qty_sum_map.get(int(booking.id), 0) or _booking_passenger_count(db, booking, qty_map=qty_sum_map))),
        )
    ...
    projection_by_trip_id: dict[int, dict] = {}
    all_bucket_trip_ids = sorted({
        int(trip_id)
        for day_bucket in day_direction_map.values()
        for bucket in day_bucket.values()
        for trip_id in bucket.get("trip_ids", set())
    })

    for trip_id in all_bucket_trip_ids:
        try:
            projection_by_trip_id[int(trip_id)] = build_driver_trip_projection(db, int(trip_id))
        except Exception:
            projection_by_trip_id[int(trip_id)] = {
                "trip": trip_by_id.get(int(trip_id)),
                "passengers": [],
                "booking_pending": [],
                "booking_coverage": [],
    ...
    for date_key in sorted(day_direction_map.keys()):
        ...
        for direction_key, bucket in sorted(dirs.items(), key=lambda kv: str(kv[1].get("label") or "").lower()):
            ...
            for trip_id in sorted(bucket.get("trip_ids", set())):
                projection = projection_by_trip_id.get(int(trip_id)) or {}
                for p in projection.get("passengers") or []:
