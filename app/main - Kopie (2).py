...
def _empty_dashboard_direction_payload(label: str = "—") -> dict:
    return {
        "present": False,
        "label": label,
        "issue_count": 0,
...
def admin_bookings_dashboard_page(request: Request, db: Session = Depends(get_db)):
    ...
    if booking_ids:
        ...
        for bid, seat_no in final_seat_rows:
            if bid is not None and seat_no and int(bid) not in final_seat_map:
    ...
    raw_trip_id_rows = (
        db.query(TripPassenger.trip_id)
    ...
    raw_trip_ids = {
        int(trip_id)
        for (trip_id,) in raw_trip_id_rows
    ...
    relevant_trip_ids = sorted(raw_trip_ids | booking_trip_ids)
    trip_by_id: dict[int, Trip] = {}

    if relevant_trip_ids:
        trips = db.query(Trip).filter(Trip.id.in_(relevant_trip_ids)).all()
    ...
    def _dashboard_item(booking: Booking, dep_dt: datetime | None, booking_date_obj: date, hours_left: float | None) -> dict:
        passenger_name = f"{booking.first_name or ''} {booking.last_name or ''}".strip() or "—"
    ...
    # Seed the calendar from Excel/live trip data too, not only from Booking rows.
    for trip_id in sorted(raw_trip_ids):
        trip = trip_by_id.get(int(trip_id))
        ...
        bucket["present"] = True
        bucket["trip_ids"].add(int(trip_id))
    ...
    for booking in all_bookings:
        ...
        setattr(booking, "_dashboard_final_seat", final_seat or None)
        setattr(
            booking,
    ...
    for date_key in sorted(day_direction_map.keys()):
        ...
        legacy_red = direction_payloads[0] if len(direction_payloads) > 0 else _empty_dashboard_direction_payload()
        legacy_blue = direction_payloads[1] if len(direction_payloads) > 1 else _empty_dashboard_direction_payload()
