// build: fix7
console.log('orders_app_fix6_fix7.jsx loaded (fix7)');
...
function OrdersPage(){
  const [busOptions, setBusOptions] = React.useState(["Неразпределени","W3200MW","W3201MW","W3202MW","W3203MW","W3204MW"]);
  React.useEffect(()=>{ fetch('/buses/api/list').then(r=>r.json()).then(j=>{ try{ const plates=(j.buses||[]).map(b=>b.plate||b.reg||b.id).filter(Boolean); if(plates.length) setBusOptions(["Неразпределени", ...Array.from(new Set(plates))]); }catch{} }); },[]);
  ...
  return (<div className="space-y-4">
    ...
    {editing && (<div className="bg-white border rounded-xl p-4 space-y-6">
      ...
      <OrdersShared.OrderForm value={editing} onChange={setEditing} onSubmit={(o)=>{ save(o); setEditing(o); }} submitText="Запази" busOptions={busOptions} />
