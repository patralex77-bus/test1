...
function useDragAndDrop(){
  ...
  function onDropToCell(date, lane){
    return (e)=>{
      ...
      const firstDate = parseYMDLocal(all[0].date);
      const lastDate  = parseYMDLocal(all[all.length-1].date);
      const daysCount = Math.round((lastDate-firstDate)/(24*3600*1000)) + 1;

      const newStart = makeLocalDate(date.getFullYear(), date.getMonth(), date.getDate(), 12);
      ...
      for(let i=0;i<daysCount;i++){
        ...
        fresh.push({
          ...
          // оставяме каквито driver_* полета идват от бекенда чрез следващ синк
        });
      }
...
function LaneRow({lane, days, byDayLane, onDropCell, allowDrop, dnd, onPickStatus, setSelectedId}){
  ...
  function barClass(status, isUnassigned){
    if(isUnassigned){
      // СИГНАЛЕН стил за незачислен към шофьор
        ...
        {summary ? (
          <div className="space-y-1 text-sm">
            ...
            <div className="flex items-center gap-2 pt-1">
              {/* промяна на линка */}
              <a href="/orders/list" className="px-3 py-1.5 text-xs rounded-lg border">Отвори в „Поръчки“</a>
            </div>
          </div>
        ) : (
