                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    ...
                    <div className="bg-slate-50 rounded-lg p-2">
                      ...
                      <div>Приход: {(o.actual?.revenue??o.price??0).toFixed(2)} лв</div>
...
function MonthView({anchor, assignments, dnd, ordersById}){
  ...
  function dropOnCell(date, lane){
    return (e)=>{
      ...
      if(OS){ OS.patchOrder(String(id), { vehicle_plate: lane }); }
    };
  }
        ...
        <div className="overflow-x-auto" ref={headerRef}>
          ...
          <div className="min-w-max flex">
            <div className="h-8 flex">
              {days.map((d,i)=>(
                <div key={i} className="w-12 border-r p-1.5 shrink-0 text-[10px] text-center font-semibold">
                  {d.getDate()}
                </div>
              ))}
            </div>
                  ...
                  return (
                    <div key={i} className={cellClass} onDragOver={dnd.allowDrop} onDrop={dropOnCell(d, lane)}>
                      <div className="space-y-1">
                        {list.map(a=>(
                          <div key={a.id} draggable onDragStart={(e)=>dnd.onDragStart(e, a.id)} onDragEnd={dnd.onDragEnd} onClick={()=>onSelect(a.id)} style={{ outline: (selectedId===a.id?'2px solid #f59e0b':'') }}
        ...
        return (
          <div key={i} className="bg-white border rounded-xl p-3" onDragOver={dnd.allowDrop} onDrop={dnd.dropOnDate(d)}>
            ...
            <div className="space-y-2">
              {list.map(a=>(
                <div key={a.id} className="border rounded-lg p-2 cursor-move" draggable onDragStart={(e)=>dnd.onDragStart(e, a.id)} onDragEnd={dnd.onDragEnd} onClick={()=>onSelect(a.id)} style={{ outline: (selectedId===a.id?'2px solid #f59e0b':'') }}>
          ...
          return (
            <div key={h} className="border-t px-3 py-2 min-h-[56px]" onDragOver={dnd.allowDrop} onDrop={dnd.dropOnDateHour(anchor, h)}>
              ...
              <div className="flex flex-wrap gap-2">
                {items.map(a=>(
                  <div key={a.id} className="border rounded-lg px-2 py-1 cursor-move" draggable onDragStart={(e)=>dnd.onDragStart(e, a.id)} onDragEnd={dnd.onDragEnd} onClick={()=>onSelect(a.id)} style={{ outline: (selectedId===a.id?'2px solid #f59e0b':'') }}>
...
function App(){
  ...
  return (
    <div className="p-4">
      ...
      {view==="month" && <MonthView anchor={anchor} assignments={assignments} dnd={dnd} ordersById={ordersById} onSelect={setSelectedId} selectedId={selectedId} />}
